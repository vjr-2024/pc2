...
class DeliveryBoyOrdersView extends GetView<OrdersController> {
  const DeliveryBoyOrdersView({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final controller = Get.find<OrdersController>();

    // Apply default filtering to show "Shipped" or "Out for Delivery" orders
    controller.filterOrdersByStatuses(
        ['Shipped', 'Out for Delivery']); // Pass both statuses as a list
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (order['status'] == 'Shipped')
                      ElevatedButton(
                        onPressed: () async {
                          await _changeOrderStatus(order['documentId']);
                        },
                        style: ElevatedButton.styleFrom(
                    ...
                    if (order['status'] == 'Out for Delivery')
                      ElevatedButton(
                        onPressed: () async {
                          await _changeOrderStatusToDelivered(
                              order['documentId']);
                        },
                        style: ElevatedButton.styleFrom(
  ...
  Future<Map<String, dynamic>?> _getRouteAndETA(double originLat,
    ...
    const String apiKey = 'AIzaSyDfXX54MU-OKbx0UGTtzfwkXamvrcDz0m4';
    final String url =
  ...
  Future<void> _changeOrderStatus(String documentId) async {
    try {
      await FirebaseFirestore.instance
          .collection('orders')
          .doc(documentId)
          .update({'status': 'Out for Delivery'});
      _showSnackbar("Success", "Order status changed to Out for Delivery.");
    } catch (e) {
      _showSnackbar("Error", "Failed to update order status: $e");
    }
  }

  // New function to mark the order as delivered
  Future<void> _changeOrderStatusToDelivered(String documentId) async {
    try {
      await FirebaseFirestore.instance
          .collection('orders')
          .doc(documentId)
          .update({'status': 'Delivered'});
      _showSnackbar("Success", "Order status changed to Delivered.");
    } catch (e) {
      _showSnackbar("Error", "Failed to update order status: $e");
    }
  }
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              // TextButton(
              //   onPressed: () {
              //     controller.filterOrdersByStatus('Pending');
              ...
              TextButton(
                onPressed: () {
                  controller.filterOrdersByStatus('Shipped');
                  Navigator.pop(context);
                },
                child: Text('Shipped'),
              ),
              TextButton(
                onPressed: () {
                  controller.filterOrdersByStatus('Out for Delivery');
                  Navigator.pop(context);
                },
              ...
              TextButton(
                onPressed: () {
                  controller.filterOrdersByStatus('Delivered');
                  Navigator.pop(context);
                },
              ...
              // TextButton(
              //   onPressed: () {
              //     controller.filterOrdersByStatus('All');
              //     Navigator.pop(context);
              //   },
              //   child: Text('All'),
              // ),
